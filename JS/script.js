...
let list = document.getElementsByClassName(`nav-list`)[0].children;
...
function classRemove(elements, classElement) {
    for (let element of elements) {
...
for (let i = 0; i < list.length; i++) {
    list[i].addEventListener(`click`, function(){
        ...
        classToggle(navbar, `hidden-nav`);
        classToggle(buttonContainer, `active`)
        classRemove(sections, `display-none`)
        classRemove(footer, `display-none`) 
    })    
}